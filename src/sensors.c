...
void sensors_init(SensorConfig* config)
{
    ...
    int8_t bme280ret = bme280_init(&sensors.bmedev);
    if (bme280ret == 0)
    {
    ...
    else
    {
        while (1)
        {
            ESP_LOGE(TAG, "BME280 ERROR! %d", bme280ret);
...
void sensors_update(SensorData* data)
{
    ...
    ads1115_setMux(&sensors.ads, ADS1115_MUX_AIN2_GND);
    data->vSolar = ads1115_readVolts(&sensors.ads);

    ads1115_setMux(&sensors.ads, ADS1115_MUX_AIN3_GND);
    data->vBatt = ads1115_readVolts(&sensors.ads);
